...
then
  ...
  echo "Container $container_name from $image_name image is now started on localhost$container_port"
  sudo docker exec -it $container_name bash
else
  echo "container mount failed"
fi
# note: with port 0, you ask the kernel to handle the attribution of a free port
